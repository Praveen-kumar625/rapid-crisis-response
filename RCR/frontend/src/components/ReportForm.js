...
function ReportForm() {
    ...
    // ---------------------------------------------------------
    // ---------------------------------------------------------
    // 🚨 BUG FIX: Sync pending reports with Mutex Lock (isSyncing)
    // ---------------------------------------------------------
    useEffect(() => {
        let isSyncing = false; // Mutex lock variable

        async function syncPending() {
            if (isSyncing) return; // Agar sync already chal raha hai, toh stop karein
            isSyncing = true; // Sync start hote hi lock laga dein
            ...
            try {
                ...
                isSyncing = false; // Sync complete/fail hone par lock hata dein
            }
        }
